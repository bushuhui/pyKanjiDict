...
#
#
#	$Id: WmDefault.tcl,v 1.1.2.10 2002/12/02 04:02:51 idiscovery Exp $
#
...
proc tixDetermineWM {} {
    ...
    set type ""
    switch -exact -- $tcl_platform(os) {Windows 95} - {Windows NT} {
	set type windows
    } default {
...
namespace eval ::wm_default {
    ...
    if {$_usetix} {lappend _system_font_widgets \
   ...
   proc debug {str} {
       ...
       if {[info commands "tix"] != "" && \
	       ([tix cget -debug] == "" || [tix cget -debug] == "0")} {return}
    ...
    # Does anyone know what these should be for Unix and Windows?
    ...
    switch -exact -- $tcl_platform(platform) windows {
	# unfortunately CYGWIN is treated as windows
